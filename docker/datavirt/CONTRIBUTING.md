...
```bash
make test VERSION=6.3 SKIP_SQUASH=1
```
...
Setting `VERSION=6.3` equally saves development time. Instead of building every
...
```bash
make VERSION=6.3 SKIP_SQUASH=1
```
...
Supposing that changes were made to files in the 6.3 directory, we can then
apply those changes to all other images:

```bash
for version in 2.4 2.6 3.0-upg; do
  git diff -- 6.3 | git apply -p2 --directory ${version} --reject
done
```
...
Eash version of JDV maintains their own test cases within `test/`. To run the tests, run:
...
* Run all tests on a single version of JDV.

  ```
  make test VERSION=<jbossdv-version>
  ```
* Run all tests on a single base image version of JDV.

  ```
  make test VERSION=<jbossdv-version> TARGET=<base-image>
  ```
...
* Run all OpenShift tests on a single version of JDV.

  ```
  make test-openshift VERSION=<jbossdv-version>
  ```
* Run all OpenShift tests on a single base image version of JDV.

  ```
  make test-openshift VERSION=<jbossdv-version> TARGET=<base-image-os>
  ```
...
[1]: https://github.com/ecwpz91/jbossdv-container/issues
