...
```bash
make test VERSION=15 SKIP_SQUASH=1
```
...
Setting `VERSION=15` equally saves development time. Instead of building every
...
Supposing that changes were made to files in the 15 directory, we can then
apply those changes to all other images:

```bash
for version in 15; do
  git diff -- 15 | git apply -p2 --directory ${version} --reject
done
```
...
[1]: https://github.com/sclorg/mssql-container/issues
