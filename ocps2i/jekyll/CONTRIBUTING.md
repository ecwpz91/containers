...
```bash
make test VERSION=35 SKIP_SQUASH=1
```
...
Setting `VERSION=35` equally saves development time. Instead of building every
...
Supposing that changes were made to files in the 35 directory, we can then
apply those changes to all other images:

```bash
for version in 35; do
  git diff -- 35 | git apply -p2 --directory ${version} --reject
done
```
...
Eash version of Jekyll maintains their own test cases within `test/`. To run the tests, run:
...
* Run all tests on a single version of Jekyll.

  ```
  make test VERSION=<mssql-version>
  ```
* Run all tests on a single base image version of Jekyll.

  ```
  make test VERSION=<mssql-version> TARGET=<base-image>
  ```
...
* Run all OpenShift tests on a single version of Jekyll.

  ```
  make test-openshift VERSION=<mssql-version>
  ```
* Run all OpenShift tests on a single base image version of Jekyll.

  ```
  make test-openshift VERSION=<mssql-version> TARGET=<base-image-os>
  ```
...
[1]: https://github.com/ecwpz91/containers/issues
